...
export const BENCHMARK_DATA: BenchmarkEntry[] = [
  ...
  },
  {
    ...
    cost_saving_pct: 0,
    perf_delta: "Live mainnet beta since 28 May 2026 — priced at or below official maker rates; no independent third-party benchmark published",
    benchmark_summary: "gm (SN28) is a confidential, OpenAI-compatible LLM inference gateway built by Taostats \u2014 the team behind taostats.io, Bittensor's primary block explorer and data provider. That operator identity is the strongest single credibility signal here: the miner CLI authenticates via Taostats device-code OAuth, and the subnet's repos sit in the same GitHub org as the Taostats API tooling. Buyers point an existing OpenAI, Anthropic or Gemini SDK at the gateway and get identical behaviour; miners supply upstream frontier-model API capacity from inside an Intel TDX enclave deployed on Phala Cloud and earn the spread. Every request runs through a TDX-attested gateway with Intel-signed measurements and RA-TLS certificates, so neither the operator nor the host can read prompts or miners' upstream keys. Two privacy tiers: frontier models (the model maker still sees the prompt) and confidential models (nobody does). 38 models live including Claude Opus, GPT-5.5, Gemini 2.5 Pro and Kimi K3, at or below official provider rates. Eight upstream provider routes (Anthropic/Bedrock, OpenAI/Azure, Gemini, Chutes, Z.ai, Moonshot, DeepInfra, KubeTEE). Works with Cursor, Cline, Claude Code and the Vercel AI SDK. Unusually for a subnet it accepts ordinary payment rails \u2014 USDC and bank cards \u2014 rather than requiring TAO. Active multi-repo development: gm-miner (Rust), gm-validator (Python), gm-mcp (TypeScript) and gm-docs. Revenue: pre-revenue \u2014 public mainnet beta, no revenue or user figures disclosed. Caveats: the privacy guarantee rests entirely on TEE attestation, with no third-party privacy or performance audit published; miners resell frontier-model API capacity, which may conflict with upstream provider terms of service; no uptime or latency statistics published, which is what keeps this below Engy (SN53) in the same category.",
