...
export const BENCHMARK_DATA: BenchmarkEntry[] = [
  ...
  },
  {
    ...
    cost_saving_pct: 0,
    perf_delta: "Live mainnet beta since 28 May 2026. 38 models verified serving via the public /v1/models endpoint on 2026-08-05 (23 frontier, 13 confidential TEE, 2 open) at or below official maker rates. No independent third-party benchmark, and no uptime or latency figures published.",
    benchmark_summary: "gm (SN28) is a confidential, OpenAI-compatible LLM inference gateway built by Taostats \u2014 the team behind taostats.io, Bittensor's primary block explorer and data provider. That operator identity is the strongest single credibility signal here: the miner CLI authenticates via Taostats device-code OAuth, and the subnet's repos sit in the same GitHub org as the Taostats API tooling. Buyers point an existing OpenAI, Anthropic or Gemini SDK at the gateway and get identical behaviour; miners supply upstream frontier-model API capacity from inside an Intel TDX enclave deployed on Phala Cloud and earn the spread. Every request runs through a TDX-attested gateway with Intel-signed measurements and RA-TLS certificates, so neither the operator nor the host can read prompts or miners' upstream keys. VERIFIED LIVE (2026-08-05, direct read of api.saygm.com/v1/models): 38 models serving, split 23 frontier / 13 confidential-tier TEE / 2 open \u2014 so the privacy tier is shipped and serving, not roadmap. Confidential models include Kimi K3, DeepSeek V4-Flash, Qwen3.5-397B and GLM-5.1, all TEE-hosted. Six upstream provider routes (OpenAI, Anthropic, Gemini, Chutes, Moonshot, Z.ai) behind five API shapes (chat.completions, messages, responses, generateContent, streamGenerateContent), so Claude Code, Codex, Cursor and the Vercel AI SDK all work unmodified. The billing surface is the strongest engineering tell: an 11-dimension price vector per model covering cache read, cache write at 5m and 1h TTLs, cache storage per hour, audio in/out and separate long-context input/output tiers above a per-model token threshold \u2014 that is parity with how the frontier labs themselves bill, not a flat per-token toy. Development is steady rather than sporadic: 93 commits to gm-miner in the 90 days to 2026-08-04 across two authors, peaking at 65 in July. Accepts ordinary payment rails (USDC and bank cards) rather than requiring TAO. Revenue: pre-revenue \u2014 public mainnet beta, no revenue or user figures disclosed. Caveats: no uptime, latency or SLA figures are published anywhere on the site or docs, which is the single biggest gap for an inference gateway and the main thing still holding the score out of the 80s; the privacy guarantee rests entirely on TEE attestation with no third-party privacy or security audit published; miners resell frontier-model API capacity, which may conflict with upstream provider terms of service; GitHub stars are near zero, though that is weak evidence either way for B2B infrastructure.",
    active_users: "pre-revenue \u2014 public mainnet beta since 28 May 2026; 38 models live, priced at or below official maker rates; no user or revenue figures disclosed",
    annual_revenue_usd: 0,
    ...
    sources: [
      ...
      "https://github.com/taostat/gm-validator",
      "https://github.com/taostat/gm-mcp",
      "https://x.com/say_gm_",
      "https://discord.com/invite/Ms2uGRWbcm",
      "https://taostats.io/subnets/28",
      "https://subnetaiq.io/subnet/28"
    ],
    dashboards: [
      { url: "https://saygm.com/", label: "Live Models & Pricing" },
      { url: "https://taostats.io/subnets/28", label: "Subnet Stats" },
    ],
  },
  {
    subnet_id: 55,
...
export const PRODUCT_MILESTONES: ProductMilestone[] = [
  ...
  },
  {
    ...
    subnet_name: "gm",
    stage: "beta",
    milestone_date: "2026-05-28",
    title: "Confidential frontier-model gateway live on mainnet — built by Taostats, 38 models, Intel TDX attestation",
    description: "gm (SN28) is a confidential LLM inference gateway from Taostats, the team behind Bittensor's primary block explorer. Buyers point an existing OpenAI, Anthropic or Gemini SDK at the gm endpoint and get identical behaviour, while every request is proxied through an Intel TDX enclave on Phala Cloud so neither the operator nor the host can read prompts or upstream API keys. 38 models live including Claude Opus, GPT-5.5, Gemini 2.5 Pro and Kimi K3, priced at or below official maker rates, with USDC and card payments rather than TAO-only. NOTE: this netuid previously ran the Foundry S&P 500 Oracle; the slot was recycled and that product is unrelated to the current occupant.",
    source_url: "https://saygm.com/",
    estimated_arr_usd: 0,
    confidence: "medium",
  },
  {
